        ...
        style={{backgroundImage:`url(${movie.imageLandscapeMobile})`,backgroundRepeat:"no-repeat",backgroundSize: "cover"}}>
            <div style={{backgroundImage:"linear-gradient(to right,rgba(0,0,0,1)150px,rgba(0,0,0,.6)100%)"}} className=''>
                <div className=' xl:max-w-7xl grid grid-cols-12 gap-x-4 mx-auto py-16'>
                    <div className='image-portrait col-span-6 md:col-span-3 relative flex justify-center items-center'>
                        <img className='mx-5 md:mx-0 max-w-[100px] md:max-w-[200px] rounded-md' src={movie.imagePortrait} alt="imageMovie" />
                    ...
                    <section className='text-white mx-5 md:mx-0 col-span-12 md:col-span-9'>
                        <h2 className='text-xl mt-2 md:text-3xl font-medium mb-3'>{movie.name}</h2>
                        <h3></h3>
                        ...
                        <ul className=''>
                            <li className='mt-1 text-sm md:text-lg'><span className='text-zinc-400  md:font-medium '>Ngày khởi chiếu: </span><span>{moment(movie.startdate).format("DD/MM/YYYY")}</span></li>
                        </ul>
                        <section className='mt-5'>
                            ...
                            <div className='text-sm md:text-lg'>
                                {
                                 <p className='text-white' dangerouslySetInnerHTML={{ __html: movie.description.replace(/color\:[\W\w]+?\"/, "color:white") }}></p>
                                }
                            </div>
                        </section>
                    </section>
